module TokenUtilsLims
  ...
  def self.is_uuid?(str)
    str.kind_of?(String) && !str.match(TokenUtilsLims.UUID_REGEXP).nil?
  end
  ...
  def self.uuid(str)
    str.match(TokenUtilsLims.UUID_REGEXP)[0]
  end

  def self.is_wildcard?(str)
    str.kind_of?(String) && !str.match(TokenUtilsLims.WILDCARD_REGEXP).nil?
  end
  ...
  def self.pad_location(location)
    return location unless location
    parts = location.match(TokenUtilsLims.LOCATION_REGEXP)
    return nil if parts.length == 0
    letter = parts[1]
    number = parts[2]
    number = TokenUtilsLims.pad(number,"0",2) unless number.length == 2
    "#{letter}#{number}"
  end
